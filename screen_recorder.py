...
class ScreenRecorder:
    ...
    def _get_audio_input_device_index(self):
        ...
        for i, device in enumerate(devices):
            if device["max_input_channels"] > 0:
                ...
                print(
                    f"  {i}: {device['name']} ({sd.query_hostapis(device['hostapi'])['name']}) (Input Channels: {device['max_input_channels']})"
                )
        ...
        if system_name == "Windows":
            ...
            preferred_keywords = [
                ...
                "loopback",
                "sum",
            ]  # Added 'sum' for some virtual cable setups
            for i, device in input_devices:
                ...
                if any(keyword in device_name_lower for keyword in preferred_keywords):
                    ...
                    print(
                        f"  --> Auto-selected Windows system audio device: {device['name']}"
                    )
                    break
            ...
            for i, device in input_devices:
                ...
                if any(keyword in device_name_lower for keyword in preferred_keywords):
                    chosen_device_index = i
                    print(
                        f"  --> Auto-selected macOS system audio device: {device['name']}. "
            ...
            for i, device in input_devices:
                ...
                if (
                    ...
                    and device["hostapi"] == sd.query_hostapis(0)["index"]
                ):  # Check if it's a PulseAudio monitor
                    chosen_device_index = i
                    print(
                        f"  --> Auto-selected Linux (PulseAudio) monitor device: {device['name']}. "
        ...
        if chosen_device_index is None:
            ...
            if default_input_device_index >= 0:
                ...
                print(
                    f"  --> No specific system audio device found. Using default input device: "
                    f"{devices[chosen_device_index]['name']} (This is likely your microphone)."
                )
            else:
                print(
                    "  --> No input audio device found at all. Audio recording will be skipped."
                )
    ...
    def _record_mic_audio(self):
        ...
        try:
            ...
            if actual_channels < self.audio_channels:
                print(
                    f"Warning: Requested {self.audio_channels} audio channels, but device '{device_info['name']}' only supports {max_input_channels}. Using {actual_channels} channels."
                )
            else:
                print(
                    f"Using {actual_channels} audio channels from device: {device_info['name']}."
                )
    ...
    def run_program_headless(self, command):
        if os.name == "nt":  # Windows
            ...
            try:
                ...
                print(f"Error: Program not found at {command}")
                return None
            except Exception as e:
                print(f"An error occurred: {e}")
                return None
    ...
    def _close(self):
        ...
        for v_index in range(len(self._video_writer)):
            ...
            if os.path.exists(temp_video_file) and os.path.exists(temp_mic_audio_file):
                ...
                print(
                    f"Merging video ({temp_video_file}) and audio ({temp_mic_audio_file}) into {outfile} using FFmpeg..."
                )
                try:
                    ffmpeg_command = [
                        "ffmpeg",
                    ...
                    self.run_program_headless(ffmpeg_command)
                    print(f"Successfully created {outfile}")
                    os.remove(temp_video_file)
                    os.remove(temp_mic_audio_file)
                except FileNotFoundError:
                    print(
            ...
            elif os.path.exists(temp_video_file) and not os.path.exists(
                temp_mic_audio_file
            ):
                print(
                    f"Only video recorded. Saving video as {self.final_output_file} (no audio)."
                )
                try:
                ...
                print(
                    "Cannot merge: One or both temporary video/audio files are missing."
                )
